---
title: "Expert: Head Motion Analysis"
execute: 
  eval: true
format: 
  html: 
...
experts_head <- filtered_data %>% 
  select(1,2,6,7,8)

experts_head$Assessment_ID <- as.factor(experts_head$Assessment_ID)
...
p1 <- ggplot(experts_head, aes(x = TimeStamp, y = HeadPositionY, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
...
p1_interactive$data[[1]]$text <- paste("Timestamp:", experts_head$TimeStamp, "<br>Y Position:", experts_head$StylusPositionY)
...
p2 <- ggplot(experts_head, aes(x = TimeStamp, y = HeadPositionX, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Time Series of Head Position",
       x = "Timestamp",
...
# Update the text attribute for the interactive plot to include formatted timestamps and Y positions
p2_interactive$data[[1]]$text <- paste("Timestamp:", experts_head$TimeStamp, "<br>X Position:", experts_head$HeadPositionX)
...
# Plotting
p3 <- ggplot(experts_head, aes(x = TimeStamp, y = HeadPositionZ, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Time Series of Head Positions",
       x = "Timestamp",
       y = "Stylus Position Z") +
  scale_color_viridis_d() +
  theme(legend.position = "right")
...
# Update the text attribute for the interactive plot to include formatted timestamps and Y positions
p3_interactive$data[[1]]$text <- paste("Timestamp:", experts_head$TimeStamp, "<br>Z Position:", experts_head$HeadPositionZ)
...
distance_matrix_expert_head = calculate_dtw_distance_xyz(df_pivoted)
...
# Convert to parquet
distance_matrix_expert_head.to_parquet('distance_matrix_expert_head.parquet')

# Upload the file
s3_client.upload_file('distance_matrix_expert_head.parquet', 'vr-sling', 'distance_matrix/XYZ/distance_matrix_expert_head.parquet')
...
plot_dtw_distance_matrix(distance_matrix_expert_head, 'dtw_expert_head_heatmap.png')

```

```{r}

write_csv(experts_head, "experts_head.csv")
...
distance_array = squareform(distance_matrix_expert_head.values, checks=False)
...
dendrogram(Z)
plt.title('Hierarchical Clustering: Expert Head Motion')
plt.xlabel('Index of Time Series')
plt.ylabel('Distance')

# Save the plot as an image file
plt.savefig('hierarchical_clustering_expert_head.png', dpi=400)
plt.show()
