---
title: "Novice: Head Motion Analysis"
execute: 
  eval: true
format: 
  html: 
...
novice_head <- novice_data %>% 
  select(1,2,6,7,8)

novice_head$Assessment_ID <- as.factor(novice_head$Assessment_ID)
...
p1 <- ggplot(novice_head, aes(x = TimeStamp, y = HeadPositionY, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Time Series of Head Position",
       x = "Timestamp",
       y = "Head Position Y") +
...
p1_interactive$data[[1]]$text <- paste("Timestamp:", novice_head$TimeStamp, "<br>Y Position:", novice_head$HeadPositionY)
...
p2 <- ggplot(novice_head, aes(x = TimeStamp, y = HeadPositionX, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Time Series of Head Position",
       x = "Timestamp",
...
# Update the text attribute for the interactive plot to include formatted timestamps and Y positions
p2_interactive$data[[1]]$text <- paste("Timestamp:", novice_head$TimeStamp, "<br>X Position:", novice_head$HeadPositionX)
...
# Plotting
p3 <- ggplot(novice_head, aes(x = TimeStamp, y = HeadPositionZ, color = Assessment_ID)) +
  geom_line() +
  theme_minimal() +
  labs(title = "Time Series of Head Position",
       x = "Timestamp",
...
# Update the text attribute for the interactive plot to include formatted timestamps and Y positions
p3_interactive$data[[1]]$text <- paste("Timestamp:", novice_head$TimeStamp, "<br>Z Position:", novice_head$HeadPositionZ)
...
# Pivot the data
df_pivoted <- novice_head %>%
  group_by(Assessment_ID) %>%
  summarise(posX_list = list(HeadPositionX),
            posY_list = list(HeadPositionY),
            posZ_list = list(HeadPositionZ))
...
distance_matrix_novice_head = calculate_dtw_distance_xyz(df_pivoted)
...
# Convert to parquet
distance_matrix_novice_head.to_parquet('distance_matrix_novice_head.parquet')

# Upload the file
s3_client.upload_file('distance_matrix_novice_head.parquet', 'vr-sling', 'distance_matrix/XYZ/distance_matrix_novice_head.parquet')
...
plot_dtw_distance_matrix(distance_matrix_novice_head, 'dtw_novice_head_heatmap.png')

```

```{r}

write_csv(novice_head, "novice_head_ts.csv")
...
distance_array = squareform(distance_matrix_novice_head.values, checks=False)
...
dendrogram(Z)
plt.title('Hierarchical Clustering: Novice Head Motion')
plt.xlabel('Index of Time Series')
plt.ylabel('Distance')

# Save the plot as an image file
plt.savefig('hierarchical_clustering_novice_head.png', dpi=400)
