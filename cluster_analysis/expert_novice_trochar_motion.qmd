...
vr_sling_ds <- vr_sling_ds %>%
  mutate(
    Group = case_when(
      str_detect(Assessment_ID, "novice") ~ "Novice",
      TRUE ~ "Expert"
    )
  )
...
all_trochar <- vr_sling_ds %>% 
  select(1,2,3,4,5,27)

all_trochar$Assessment_ID <- as.factor(all_trochar$Assessment_ID)
...
distance_matrix_all_trochar = calculate_dtw_distance_xyz(df_pivoted)
...
# Convert to parquet
distance_matrix_all_trochar.to_parquet('distance_matrix_all_trochar.parquet')

# Upload the file
s3_client.upload_file('distance_matrix_all_trochar.parquet', 'vr-sling', 'distance_matrix/XYZ/distance_matrix_all_trochar.parquet')
...
plot_dtw_distance_matrix(distance_matrix_all_trochar, 'dtw_all_trochar_heatmap.png')

```

```{r}

write_csv(all_trochar, "all_trochar_ts.csv")
...
distance_array = squareform(distance_matrix_all_trochar.values, checks=False)
...
dendrogram(Z)
plt.title('Hierarchical Clustering: Expert and Novice Trochar Motion Analysis')
plt.xlabel('Index of Time Series')
plt.ylabel('Distance')

# Save the plot as an image file
plt.savefig('hierarchical_clustering_all_trochar.png', dpi=400)
...
df_pivoted['Group'] = df_pivoted['Assessment_ID'].apply(
    lambda x: 'Novice' if 'novice' in x.lower() else 'Expert'
)

# Assign cluster labels based on a chosen threshold
max_d = 150
df_pivoted['Cluster'] = fcluster(Z, max_d, criterion='distance')

# Analyze the clusters by the original groups
cluster_group_analysis = df_pivoted.groupby(['Cluster', 'Group']).size().unstack(fill_value=0)

# Display the analysis of groups within each cluster
print(cluster_group_analysis)

```

```{r}
group_clusters <- py$cluster_group_analysis
```
